...
/* Conversion Comments
   ...
   ************************************************************
   ** INIT   *  INITIAL VERSION   *  2021-11-16 12:36:00 PM
...
namespace GOV.KS.DCF.CSS.Online.BL
{
    ...
    public class SWEXGCPR : BatchBase
    {
        ...
        {
            //COMMENT: -----------------------------------------------------------------
            DbConv.SetQueryText("SPA-VIEW", "SELECT SYSTEM_CODE, PARTICIPATION_FLAG FROM KSD_SYSTEM_PART WHERE FK_CLB_CLIENT_NO = {0}  AND FK_CLI_CLIENT_NAME = {1}  AND FK_CLI_SSN = {2}  WITH UR",  //COBOL==> EXEC SQL DECLARE SPA-VIEW CURSOR FOR SELECT SYSTEM_CODE , PARTICIPATION_FLAG FROM KSD_SYSTEM_PART WHERE FK_CLB_CLIENT_NO = :DBK1-CLIENT-NUMBER AND FK_CLI_CLIENT_NAME = :DBK1-CLIENT-NAME AND FK_CLI_SSN = :DBK1-SSN WITH UR END-EXEC.
        ...
        {
            //COMMENT: -----------------------------------------------------------------
            DbConv.ExecuteSqlQuery("SELECT VERIFIED_SSN_FLAG INTO {0} FROM KSD_CLIENT WHERE FK_CLB_CLIENT_NO = {1} AND CLIENT_NAME = {2} AND SSN = {3} WITH UR ",  //COBOL==> EXEC SQL SELECT VERIFIED_SSN_FLAG INTO :CLI-VERIFIED-SSN-FLAG FROM KSD_CLIENT WHERE FK_CLB_CLIENT_NO = :DBK1-CLIENT-NUMBER AND CLIENT_NAME = :DBK1-CLIENT-NAME AND SSN = :DBK1-SSN WITH UR END-EXEC.
            ...
            if ((WS.SQLCA.SQLCODE.IsEqualTo(100)))
            {
                ...
                WS.WSO_RETURN_SQLCODE.SetValue(WS.SQLCA.SQLCODE);                                                   //COBOL==> MOVE SQLCODE TO WSO-RETURN-SQLCODE
            ...
            else
            {
                ...
                WS.WSO_ADA_RESPONSE_CD.SetValue(WS.SQLCA.SQLCODE);                                                  //COBOL==> MOVE SQLCODE TO WSO-ADA-RESPONSE-CD
                                                                                                                    //COMMENT: ** GQB DISPLAY SQL ERROR MESSAGE ***
