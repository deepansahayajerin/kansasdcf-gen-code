...
/* Conversion Comments
   ...
   ************************************************************
   ** INIT   *  INITIAL VERSION   *  2022-03-02 04:51:16 PM
...
namespace GOV.KS.DCF.CSS.Common.BL
{
    ...
    public class SWEXIR10 : OnlineEABBase
    {
        ...
        private void SetUpProgram()
        {
            ...
            DbConv.SetQueryTextWithUR("SYSPART", "SELECT SYSTEM_CODE, PARTICIPATION_FLAG FROM KSD_SYSTEM_PART WHERE FK_CLB_CLIENT_NO = {0}  AND FK_CLI_CLIENT_NAME = {1}  AND FK_CLI_SSN = {2} ",  //COBOL==>EXEC SQL DECLARE SYSPART CURSOR FOR SELECT SYSTEM_CODE , PARTICIPATION_FLAG FROM KSD_SYSTEM_PART WHERE FK_CLB_CLIENT_NO = :SPART-FK-CLB-CLIENT-NO AND FK_CLI_CLIENT_NAME = :SPART-FK-CLI-CLIENT-NAME AND FK_CLI_SSN = :SPART-FK-CLI-SSN WITH UR END-EXEC
        ...
        {
            DbConv.ExecuteSqlQueryWithUR("SELECT VERIFIED_SSN_FLAG INTO {0} FROM KSD_CLIENT WHERE FK_CLB_CLIENT_NO = {1} AND CLIENT_NAME = {2} AND SSN = {3} ",  //COBOL==> EXEC SQL SELECT VERIFIED_SSN_FLAG INTO :CLIENT-VERIFIED-SSN-FLAG FROM KSD_CLIENT WHERE FK_CLB_CLIENT_NO = :CLIENT-FK-CLB-CLIENT-NO AND CLIENT_NAME = :CLIENT-CLIENT-NAME AND SSN = :CLIENT-SSN WITH UR END-EXEC.
        ...
        {
            // Declare Relocated to Method SetUpProgram!                                                        //COBOL==> EXEC SQL DECLARE SYSPART CURSOR FOR SELECT SYSTEM_CODE , PARTICIPATION_FLAG FROM KSD_SYSTEM_PART WHERE FK_CLB_CLIENT_NO = :SPART-FK-CLB-CLIENT-NO AND FK_CLI_CLIENT_NAME = :SPART-FK-CLI-CLIENT-NAME AND FK_CLI_SSN = :SPART-FK-CLI-SSN WITH UR END-EXEC.
            // EvaluateList !DbConv.SQLCA.SQLCODE!                                                              //COBOL==> EVALUATE SQLCODE
            if ((DbConv.SQLCA.SQLCODE.IsZeroes()))                                                              //COBOL==> WHEN ZERO
            {
                //Continue                                                                                          //COBOL==> CONTINUE
